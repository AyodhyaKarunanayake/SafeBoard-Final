...
import '../services/seat_release_service.dart';
...
class TicketsProvider with ChangeNotifier {
  ...
  void endJourney(String ticketId) {
    ...
    // Let the conductor app know the seat(s) are free again (fire-and-forget).
    // A bulk ticket ends for the whole group, so every seat on it is released.
    final releaseService = SeatReleaseService();
    for (final allocation in _tickets[index].allAllocations) {
      releaseService.release(allocation.allocationId);
    }
    notifyListeners();
  }
